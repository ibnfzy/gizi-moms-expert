...
const initAdminMothers = () => {
    ...
    const formatValue = (value, suffix = '') => {
        ...
        const text = typeof value === 'number' && !Number.isNaN(value) ? value : String(value);
    ...
    const renderRows = (items) => {
        ...
        const rows = items.map((mother) => {
            if (mother?.id !== undefined) {
                motherStore.set(String(mother.id), mother);
            }

            const status = mother?.status || {};
            const badgeClass = typeof status.badge === 'string' && status.badge.trim() !== ''
                ? status.badge
            ...
            return `
                <tr class="transition hover:bg-gray-50">
                    <td class="px-6 py-4 font-medium text-gray-900">${escapeHtml(mother?.name ?? '-')}</td>
                    <td class="px-6 py-4 text-gray-700">${escapeHtml(mother?.email ?? '-')}</td>
                    <td class="px-6 py-4 text-gray-600">${escapeHtml(formatValue(mother?.umur))}</td>
                    <td class="px-6 py-4 text-gray-600">${escapeHtml(formatValue(mother?.usia_bayi_bln))}</td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${badgeClass}">${escapeHtml(badgeLabel)}</span>
                    </td>
                    <td class="px-6 py-4 text-right text-sm">
                        <div class="flex flex-wrap justify-end gap-2">
                            <button type="button" class="rounded-lg border border-blue-200 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-50" data-action="detail" data-id="${escapeHtml(mother?.id)}">Detail</button>
                            <button type="button" class="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50" data-action="email" data-id="${escapeHtml(mother?.id)}">Edit Email</button>
                            <button type="button" class="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50" data-action="password" data-id="${escapeHtml(mother?.id)}">Atur Password</button>
                            <button type="button" class="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" data-action="delete" data-id="${escapeHtml(mother?.id)}">Hapus</button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    const populateDetail = (data) => {
        ...
        if (detailElements.usiaBayi) {
            detailElements.usiaBayi.textContent = formatValue(profile.usia_bayi_bln, ' bln');
        }
        if (detailElements.laktasi) {
            detailElements.laktasi.textContent = typeof profile.laktasi_tipe === 'string' && profile.laktasi_tipe !== ''
                ? profile.laktasi_tipe.toUpperCase()
                : '-';
        }
        if (detailElements.aktivitas) {
            detailElements.aktivitas.textContent = typeof profile.aktivitas === 'string' && profile.aktivitas !== ''
        ...
        if (detailElements.inference) {
            if (!latestInference) {
                ...
                const timestamp = latestInference.created_at_human || latestInference.created_at || '-';
                const statusLabel = (latestInference.status && latestInference.status.label)
                    || status.label
                    || 'Tidak diketahui';
    ...
    const handleDetail = async (id) => {
        try {
            ...
            showNotification(notificationId, 'error', error.message || 'Gagal memuat detail ibu.');
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        const confirmation = window.confirm(`Hapus data ibu "${mother.name ?? 'Tanpa Nama'}"?`);
        if (!confirmation) {
            return;
        }
        ...
        try {
            ...
            showNotification(notificationId, 'error', error.message || 'Gagal menghapus data ibu.');
        }
    };
    ...
    const loadMothers = async () => {
        ...
        try {
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="6" class="px-6 py-6 text-center text-sm text-red-600">${escapeHtml(error.message || 'Gagal memuat data ibu.')}</td>
                </tr>
            `;
            showNotification(notificationId, 'error', error.message || 'Gagal memuat data ibu.');
        }
    };
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                closeEmailModal();
                showNotification(notificationId, 'success', 'Email ibu berhasil diperbarui.');
                await loadMothers();
            } catch (error) {
                showNotification(notificationId, 'error', error.message || 'Gagal memperbarui email ibu.');
            }
        });
    }
    ...
    if (passwordForm) {
        passwordForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                closePasswordModal();
                showNotification(notificationId, 'success', 'Password ibu berhasil diperbarui.');
            } catch (error) {
                showNotification(notificationId, 'error', error.message || 'Gagal memperbarui password ibu.');
                return;
            }
