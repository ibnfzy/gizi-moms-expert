...
const registerPakarComponents = (alpineInstance) => {
  ...
  alpineInstance.data("pakarDashboard", (config = {}) => ({
    ...
    loading:
      !Array.isArray(config.initialMothers) ||
      config.initialMothers.length === 0,
    detailOpen: false,
    detailLoading: false,
    selectedMother: null,
    init() {
      ...
      this.motherDetailEndpoint =
        dataset.motherDetailEndpoint || "/api/mothers";
  ...
  alpineInstance.data("pakarConsultation", () => ({
    ...
    async fetchConsultations() {
      ...
      try {
        ...
        showNotification(
          this.notificationId,
          "error",
          error.message || "Gagal memuat daftar konsultasi."
        );
      } finally {
    ...
    updateConsultation(id, conversation) {
      ...
      merged.messages = Array.isArray(messages)
        ? messages
        : existing.messages ?? [];
      merged.last_message =
        conversation?.last_message ??
        merged.messages?.slice(-1)[0] ??
    ...
    replaceMessage(tempId, actualMessage) {
      ...
      const index = this.selected.messages.findIndex(
    ...
    async sendMessage() {
      if (!this.selectedId) {
        this.feedback = {
          type: "warning",
          message: "Pilih sesi konsultasi terlebih dahulu.",
        };
        return;
      }
