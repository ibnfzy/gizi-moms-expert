...
>
    ...
    <div class="relative z-10 w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900 dark:text-slate-200">
        ...
        <form
            ...
            data-schedule-create-form
            data-submit-url="<?= esc($createEndpoint) ?>"
        >
            <div
            ...
            ></div>
            <div>
                <label for="schedule-create-mother" class="text-sm font-medium text-slate-700 dark:text-slate-200">Ibu Konsultasi</label>
                <select
                    id="schedule-create-mother"
                    name="mother_id"
                    required
                ...
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Daftar diambil dari profil ibu yang telah terdaftar.</p>
            </div>
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label for="schedule-create-datetime" class="text-sm font-medium text-slate-700 dark:text-slate-200">Waktu Konsultasi</label>
                    <input
                        type="datetime-local"
                        id="schedule-create-datetime"
                        name="scheduled_at"
                        required
                        class="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-giziblue focus:outline-none focus:ring-2 focus:ring-giziblue/40 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                ...
                <div>
                    <label for="schedule-create-status" class="text-sm font-medium text-slate-700 dark:text-slate-200">Status Jadwal</label>
                    <select
                        id="schedule-create-status"
                        name="status"
                        required
                        class="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-giziblue focus:outline-none focus:ring-2 focus:ring-giziblue/40 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                        data-create-status
                    ...
                    >
                        <?php foreach ($statusOptions as $value => $label): ?>
                            <option value="<?= esc($value) ?>" <?= $value === 'pending' ? 'selected' : '' ?>><?= esc($label) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div>
                <label for="schedule-create-location" class="text-sm font-medium text-slate-700 dark:text-slate-200">Lokasi Konsultasi</label>
                <input
                    type="text"
            ...
            <div>
                <label for="schedule-create-notes" class="text-sm font-medium text-slate-700 dark:text-slate-200">Catatan Tambahan</label>
                <textarea
                    id="schedule-create-notes"
                    name="notes"
                    rows="3"
                    placeholder="Catatan untuk persiapan konsultasi"
                    class="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-giziblue focus:outline-none focus:ring-2 focus:ring-giziblue/40 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                    data-create-notes
                ></textarea>
            </div>
