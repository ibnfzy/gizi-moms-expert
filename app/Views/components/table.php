...
?>
<div
  class="rounded-xl border border-gray-200 bg-white shadow-sm shadow-slate-100 dark:border-black/70 dark:bg-slate-950/60 dark:shadow-black/40">
  ...
  <div class="overflow-x-auto">
    <table class="min-w-full border-collapse border border-black/70 dark:border-gray-300 dark:bg-slate-950/40">
      ...
      <thead class="bg-gray-50 dark:bg-slate-950/60">
        <tr>
          <?php foreach ($normalizedHeaders as $header) :
                            ...
                            $thClass = trim('border border-black/70 px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider dark:border-gray-300 dark:text-slate-300 ' . ($alignClasses[$align] ?? $alignClasses['left']) . ' ' . $extraClass);
                        ?>
      ...
      <tbody class="bg-white dark:bg-slate-950/30 dark:text-slate-200">
        <?php if (empty($rows)) : ?>
        <tr>
          <td colspan="<?= count($normalizedHeaders) ?>"
            class="border border-black/40 px-6 py-4 text-center text-sm text-gray-500 dark:border-gray-300 dark:text-slate-400">
            <?= esc($emptyMessage) ?>
          </td>
        </tr>
        <?php else : ?>
        ...
        <tr class="<?= esc($rowClass) ?>">
          <?php foreach ($cells as $index => $cell) :
                                ...
                                $tdClass = trim('border border-black/40 px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:border-gray-300 dark:text-slate-200 ' . ($alignClasses[$cellAlign] ?? $alignClasses['left']) . ' ' . $cellClass);
                            ?>
